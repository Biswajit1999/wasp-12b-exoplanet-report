...
def test_pipeline_reproduces_committed_statistics():
    analysis.main()
    values = read_summary()
    assert np.isclose(values["depth_ppm"], 12579.0193705, rtol=0, atol=0.1)
    assert np.isclose(values["chi_square_flat"], 18703.876136, rtol=1e-9)
    assert np.isclose(values["chi_square_box"], 4428.1045155, rtol=1e-9)
    assert np.isclose(values["delta_chi_square"], 14275.7716205, rtol=1e-9)
